...
# ✅ Debugging Mode
DEBUG_MODE = True  # इसे True रखो, ताकि logs दिखें
...
async def tag_all(client, message: Message):
    ...
    for i in range(0, len(members), 5):
        ...
        # 🔥 Debugging - यूजर्स के नाम और ID चेक करें
        for user in group:
            print(f"DEBUG: {user.first_name} (ID: {user.id})")

        # ✅ None Check ताकि बॉट क्रैश न हो
        text = "🔥 **Attention Everyone!** 🔥\n\n" + " ".join(
            [f"[{user.first_name if user and user.first_name else 'Unknown'}](tg://user?id={user.id})" for user in group if user and user.id]
        )
